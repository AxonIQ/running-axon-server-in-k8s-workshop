...
if [ ! -s ssl/internal-ca.crt ] ; then
    ...
    ${BINDIR}/gen-ca.sh -o ssl/internal-ca ${NS_NAME}.svc.cluster.local
fi
...
LICENSE=./axoniq.license
while [ ! -s ${LICENSE} ] ; do
    echo ""
    echo "You need a license file to run Axon Server EE."
    echo "Enter path to the license file [./axoniq.license]: " ; read file
    if [[ "${file}" == "" ]] ; then
        file=./axoniq.license
    fi
